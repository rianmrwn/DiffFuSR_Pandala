...
from litsr.data.single_image_to_patch_dataset import SingleImageToPatchDataset,SingleRawImageToPatchDataset,DoubleRawImageToPatchDataset,SingleRaw6ImageToPatchDataset
    ...
    def setup(self, stage: Optional[str]):
        if stage == "fit":
            self.train_dataset = SingleImageToPatchDataset(
                ...
                is_train=True,
                multi_spectral=self.opt.get("multi_spectral"), channels=self.opt.get("channels"),
...
@DataModuleRegistry.register()
class BlindSRJIFUSGSDataModule(LightningDataModule):
    def __init__(self, opt):
        super().__init__()
        self.opt = opt
    ...
    def setup(self, stage: Optional[str]):
        if stage == "fit":
            self.train_dataset = SingleImageToPatchDataset(
                ...
                std=self.opt.get("std"),
                return_img_name=False,
                harmonization=self.opt.train.get("harmonization"),
            )
        # self.val_dataset = SingleImageToPatchDataset(
        #     img_path=self.opt.valid.data_path,
        #     is_train=False,
        #     rgb_range=self.opt.rgb_range,
        #     multi_spectral=False,channels=3,
        #     first_k=self.opt.valid.get("data_first_k"),
        #     mean=self.opt.get("mean"),
        #     std=self.opt.get("std"),
        #     return_img_name=True,
...
@DataModuleRegistry.register()
class BlindSRJIFSPOTRAWDataModule(LightningDataModule):
    def __init__(self, opt):
        super().__init__()
        self.opt = opt
    ...
    def setup(self, stage: Optional[str]):
        if stage == "fit":
            self.train_dataset = SingleRawImageToPatchDataset(
                img_path=self.opt.train.data_path,
                is_train=True,
                multi_spectral=self.opt.get("multi_spectral"), channels=self.opt.get("channels"),
                repeat=self.opt.train.get("data_repeat"),
