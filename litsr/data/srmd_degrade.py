...
class SRMDPreprocessingOpenSR(nn.Module):
    def __init__(
        self,
        scale,
        mode="bilinear",
    ...
    def forward(self, hr_tensor, random=True):
        ...
        if self.mode == "bicubic":
            ...
            lr_blured = F.interpolate(hr_blured, scale_factor=1/self.scale, mode='bilinear').squeeze()
        elif self.mode == "s-fold":
            lr_blured = hr_blured.view(
                -1, C, H // self.scale, self.scale, W // self.scale, self.scale
            )[:, :, :, 0, :, 0]
        ...
        if self.noise > 0: # TODO
            # add noise 
            rnoisy = torch.normal(0, self.noise, size=lr_blured.shape).to(lr_blured.device)
            noise_vec = torch.sqrt(torch.mean(lr_blured ** 2, dim=(1, 2), keepdim=True)) * rnoisy
            lr_blured = lr_blured + noise_vec
        lr_blured = torch.clamp(lr_blured.round(), 0, 255)
        #print(lr_blured.shape)
        if flag:
            return (
                lr_blured.view(B, C, H // int(self.scale), W // int(self.scale)),
                b_kernels,
