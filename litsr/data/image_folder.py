...
from litsr.utils import load_file_list, read_image, read_tiff
...
class Raw6xImageFolder(data.Dataset):
    """
    Construct a dataset from a folder
    """

    def __init__(self, datapath, multi_spectral=False, channels = 12, repeat=1, cache=None, first_k=None, data_length=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.cache == "memory":
            ...
            with open(x, "rb") as f:
                x = pickle.load(f)
                # if x data type is uint32 divide by 10000 and assume 
                # if x.dtype =='int32':
                #     x = x/10000
                #     # Step 1: Clamp values between 0 and 1
                #     x = np.clip(x, 0, 1)

                #     # Step 2: Scale the values from 0 to 255
                #     x = x * 255

                #     # Step 3: Convert the data type to uint8
                #     x = x.astype(np.uint8)
            return x
            ...
            if self.multi_spectral is True:
                return read_tiff(x,self.channels)
            else:
                return read_image(x)
    ...
    def load_data_from_lmdb(self, first_k=None):
        ...
        if self.multi_spectral is True:
            file_names = load_file_list(self.datapath, regx=".*.tif{1,2}")
        else:
        ...
        for f in file_names:
            ...
            if not osp.exists(bin_file):
                with open(bin_file, "wb") as bin_f:
                    if self.multi_spectral is True:
                        im_file = read_tiff(osp.join(self.datapath, f),self.channels)
                        # downsample the image 1.5 m GSD to 2 m GSD i.e. 1.5/2 = 0.75 using bicubic interpolation
                        im_file = cv2.resize(im_file, (int(im_file.shape[1] * 0.8999999999999998), int(im_file.shape[0] * 0.8999999999999998)), interpolation=cv2.INTER_CUBIC)
                        pickle.dump(im_file, bin_f,)
                    else:
                        im_file = read_image(osp.join(self.datapath, f))
                        im_file = cv2.resize(im_file, (int(im_file.shape[1] * 0.8999999999999998), int(im_file.shape[0] * 0.8999999999999998)), interpolation=cv2.INTER_CUBIC)
                        pickle.dump(im_file,bin_f,)
                print("dump", bin_file)
    ...
    def load_data_from_memory(self, first_k=None, from_pickle=True):
        ...
        for f in pbar:
            if self.multi_spectral is True:
                files.append(read_tiff(osp.join(self.datapath, f),self.channels))
            else:
...
class RawImageFolder(data.Dataset):
    """
    Construct a dataset from a folder
    """
        ...
        for f in file_names:
            ...
            if not osp.exists(bin_file):
                with open(bin_file, "wb") as bin_f:
                    if self.multi_spectral is True:
                        im_file = read_tiff(osp.join(self.datapath, f),self.channels)
                        # downsample the image 1.5 m GSD to 2 m GSD i.e. 1.5/2 = 0.75 using bicubic interpolation, fixed this error to 1.5/2.5 0.6 
                        im_file = cv2.resize(im_file, (int(im_file.shape[1] * 0.6), int(im_file.shape[0] * 0.6)), interpolation=cv2.INTER_CUBIC)
                        pickle.dump(im_file, bin_f,)
                    else:
                        im_file = read_image(osp.join(self.datapath, f))
                        im_file = cv2.resize(im_file, (int(im_file.shape[1] * 0.6), int(im_file.shape[0] * 0.6)), interpolation=cv2.INTER_CUBIC)
    ...
    def load_data_from_bin(self, first_k=None):
        ...
        for f in file_names:
            ...
            if not osp.exists(bin_file):
                with open(bin_file, "wb") as bin_f:
                    if self.multi_spectral is True:
                        pickle.dump(read_tiff(osp.join(self.datapath, f),self.channels), bin_f,)
                    else:
                        pickle.dump(read_image(osp.join(self.datapath, f)),bin_f,)
                print("dump", bin_file)
            files.append(bin_file)
        return file_names, files
...
class PairedImageFolder(data.Dataset):
    def __init__(
        self, path1, path2, repeat=1,multi_spectral= False, channels= 12, cache=None, first_k=None, data_length=None
    ):
        self.dataset_1 = ImageFolder(
            path1,multi_spectral= multi_spectral, channels=channels, repeat=repeat, cache=cache, first_k=first_k, data_length=data_length
        )
        self.dataset_2 = ImageFolder(
            path2, multi_spectral=multi_spectral, channels=channels, repeat=repeat, cache=cache, first_k=first_k, data_length=data_length
        )
