...
class Diffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop_fast_optimized(
        self,
        model,
        shape,
        device,
        ...
        skip=10,
        eta=0.0,
        include_x0_pred_freq=10,E=None,lr=None, encoder = None,
        **kwargs,
    ):
        seq = range(0, self.num_timesteps, skip)
        seq_next = [0] + list(seq[:-1])
        ...
        x = noise_fn(shape).to(device)
        x_plot = x.squeeze().cpu().numpy().transpose(1, 2, 0)
        # bring to [0, 1]
        x_plot = (x_plot - x_plot.min()) / (x_plot.max() - x_plot.min())
        plt.imsave('debug.png', x_plot)
        x0_preds = []
        for idx, i, j in zip(range(len(seq)), reversed(seq), reversed(seq_next)):
            t = torch.full((shape[0],), i, dtype=torch.int64).to(device)
            t_prev = torch.full((shape[0],), j, dtype=torch.int64).to(device)
            ...
            x_plot = pred_x0.squeeze().cpu().numpy().transpose(1, 2, 0)
            # bring to [0, 1]
            x_plot = (x_plot - x_plot.min()) / (x_plot.max() - x_plot.min())
            plt.imsave('pred_x0.png', x_plot)

            # fea = self.E(lr, lr) # k_v = fea
            ...
            x_plot = eps.squeeze().cpu().numpy().transpose(1, 2, 0)
            # bring to [0, 1]
            x_plot = (x_plot - x_plot.min()) / (x_plot.max() - x_plot.min())
            plt.imsave('eps.png', x_plot)
            ...
            optimizer = optim.Adam([x], lr=0.05* t.cpu().numpy()/1000) #0.00005
            
            ##for _ in range(int(2)):#*t_next 
                
            downscaled_hr_img =F.avg_pool2d(x, kernel_size=4, stride=4) #F.avg_pool2d(x_next, kernel_size=4, stride=4) #  F.interpolate(x_next, 64) #
            
            downscaled_lr_img = lr.clone().detach()  #F.avg_pool2d(lr_img_clean, kernel_size=4, stride=4) #F.interpolate(lr_img, 64) #F.max_pool2d(lr_img, kernel_size=4, stride=4)
            downscaled_lr_img.requires_grad = True
            
            optimizer.zero_grad()
            
            loss =  1 * loss_fn_mse(downscaled_hr_img.float(), downscaled_lr_img.float())# + 0.02* loss_fn_vgg(downscaled_hr_img.float(), downscaled_lr_img.float()) #loss_fn_vgg

            #loss =  loss_fn_mse(downscaled_hr_img, downscaled_lr_img) # loss_fn_mse(F.interpolate(x_next, 64), F.interpolate(lr_img, 64)) #
