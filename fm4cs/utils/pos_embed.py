...
# # TODO: from paper 
# def get_1d_cyclic_sincos_pos_embed_from_grid_torch(embed_dim, angles):

#     assert embed_dim % 2 == 0
#     omega = torch.arange(embed_dim // 2, dtype=torch.float32, device=angles.device)

#     angles = angles.reshape(-1)
#     angles_sin = torch.sin(angles)
#     angles_cos = torch.cos(angles)

#     out_sin = torch.einsum('m,d->md', angles_sin, omega)
#     out_cos = torch.einsum('m,d->md', angles_cos, omega)

#     emb_sin = torch.sin(out_sin)
#     emb_cos = torch.cos(out_cos)
...
def get_2d_sincos_pos_embed_with_resolution(
    ...
    if cls_token:
        pos_embed = torch.cat(
            [
                torch.zeros(
                    [n, 1, embed_dim], dtype=torch.double, device=pos_embed.device
                ),
                pos_embed,
            ],
            dim=1,
...
# DeiT: https://github.com/facebookresearch/deit
# --------------------------------------------------------
def interpolate_pos_embed_usat(model, checkpoint_model, pos_embed_key):
    if pos_embed_key in checkpoint_model:
        pos_embed_checkpoint = checkpoint_model[pos_embed_key]
        embedding_size = pos_embed_checkpoint.shape[-1]
        #num_patches = model.patch_embed.num_patches
        num_patches = model.state_dict()[pos_embed_key].shape[-2]
        num_extra_tokens = 0 # model.pos_embed.shape[-2] - num_patches # TODO: fix
        # height (== width) for the checkpoint position embedding
        orig_size = int((pos_embed_checkpoint.shape[-2] - num_extra_tokens) ** 0.5)
        # height (== width) for the new position embedding
        new_size = int(num_patches**0.5)
        ...
        if orig_size != new_size:
            ...
            pos_tokens = torch.nn.functional.interpolate(
                ...
                size=(new_size, new_size),
                mode="bicubic",
                align_corners=False,  # TODO: align corners, yes or no?
            )[0]
