...
class FM4CSIterableDatasetV2(IterableDataset, FM4CSDatasetBase):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        print("metadata dir", self.metadata_dir)
    ...
    def __iter__(self):
        #print(f"shuffling file paths epoch: {self.epoch}")
        ...
        for i in range(len(self) * self.world_size):
    ...
    def __getitem__(self, idx):
        ...
        # print("idx", idx, "file", file_path , "worker_id", worker_id, "process_rank", self.global_rank)
        ...
        if footprints_df is None:
            ...
            return self.__getitem__(idx + self.world_size * num_workers + 1)
        ...
        for sample_try in range(len(footprints_df)):
            ...
            for product in filtered_products:
                ...
                try:
                    ...
                    print(e)
                    print(file_path)
                    self._bad_hits[f"load_error_{product}"] += 1
                    self._per_file_bad_hits[str(file_path)] += 1
                    if prod_img_dataset is not None:
                        prod_img_dataset.close()
                    return self.__getitem__(idx + self.world_size * num_workers + 1)
                ...
                if (
                    ...
                    self._bad_hits[f"out_of_bounds_{product}"] += 1
                    self._per_file_bad_hits[str(file_path)] += 1
                ...
                for b in range(batch_size):
                    ...
                    # TODO: I think this method sometimes fails by all xs being the same if 'out of bounds'
                    ...
                    if prod_img.sizes["x"] != img_size or prod_img.sizes["y"] != img_size:
                        self._bad_hits[f"size_mismatch_{product}"] += 1
                        self._per_file_bad_hits[str(file_path)] += 1
                        # breakpoint()
                        found_all_products = False
                        if "S2" in product:
                    ...
                    try:
                        ...
                        self._bad_hits[f"corrupt_file_{product}"] += 1
                        self._per_file_bad_hits[str(file_path)] += 1
                        found_all_products = False
                        if "S2" in product:
                            skip_products.append(product)
                    ...
                    if (nan_ratio > self.max_nan_ratio).any():
                        self._bad_hits[f"nan_ratio_{product}"] += 1
                        self._per_file_bad_hits[str(file_path)] += 1
                    ...
                    # var_ratio = np.isnan(prod_img).sum(axis=(1, 2)) / (prod_img.shape[1] * prod_img.shape[2]) , C H W , prod_img
                    # if (nan_ratio > self.max_nan_ratio).any():
                    #     
                    #     
                    #     
        ...
        if self._hits % 25 == 0:
            ...
            # with open(f"/scratch/project_465001381/forgaard/FM4CS/NR/logs/bad_hits/bad_hits_process_{self.global_rank}_worker_{worker_id}.txt", "w") as f:
            #     for k,v in self._per_file_bad_hits.items():
            #         f.write(f"{k}:{v}\n")
