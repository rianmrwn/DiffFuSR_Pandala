...
def fast_samples_fn(
    model, diffusion, ctx, k_v, shape, device, skip=10, eta=0.0, include_x0_pred_freq=10,#E=None,lr=None,encoder=None
):
    samples, progressive_samples = diffusion.p_sample_loop_fast(#p_sample_loop_fast_optimized
        model=model,
        ctx=ctx,
        k_v=k_v,
        shape=shape,
        ...
        skip=skip,
        eta=eta,
        include_x0_pred_freq=include_x0_pred_freq,#E=E,lr=lr,encoder=encoder
    )
    return {
        "samples": samples,
...
class DiffBlindSR3AtomModel(pl.LightningModule):
    ...
    def test_step_lr_hr_paired(self, batch, *args, **kwargs):
        ...
        if approxdiff == "STEP":
            sample = fast_samples_fn(
                _model, self.diffusion, ctx, fea, hr.shape, self.device, skip, eta,#E=self.E,lr=lr,encoder = self.encoder
            )
        elif approxdiff == "VAR":
            sample = fastdpm_samples_var_fn(
                _model,
